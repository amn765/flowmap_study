...
本次未部署到线上。要随时手机访问 / 跨设备同步，可后续部署到 GitHub Pages（让我帮你一键发布即可）。

| 入口 | 本地路径 |
|------|----------|
| 📖 学习站（阅读文档 + 勾选任务） | `index.html` |
| 📊 打卡面板（进度 / 热力图 / 连续天数） | `tracker/index.html` |

## 怎么使用这套计划

1. **打开进度面板**：双击 `tracker/index.html` 即可用（不依赖联网，本地直接跑）。每完成一个任务就勾选，进度条、热力图、连续天数会实时更新。进度存在浏览器 localStorage 里，换浏览器/电脑前记得用「导出进度」备份。
2. **读文档**：双击 `index.html` 打开学习站。⚠️ 注意：本地双击时浏览器禁止用 `fetch` 读本地 `plan/*.md`（CORS），所以**学习站里的文档正文要联网部署后才看得到**；在本地想读正文，直接打开 `plan/` 文件夹里的 markdown 即可。打卡面板 `tracker/index.html` 不受此限制，本地随便用。
